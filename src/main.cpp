 ...
 *
 * @param gesture gesture that needs to be printed out
 */
...
void loop() {
  ...
  if(record){
    ...
    //NeoPixel Chaser
    CircuitPlayground.setPixelColor(dynamic_counter % 10, blue);
    CircuitPlayground.setPixelColor(dynamic_counter % 20, cyan);
    CircuitPlayground.setPixelColor(dynamic_counter % 30, purple);
    CircuitPlayground.setPixelColor(dynamic_counter % 40, red);

    //Buzzer speed reduce as time increase
    CircuitPlayground.playTone(1000, (dynamic_counter% 50)/5);
    ...
    if(dynamic_counter == COUNTER){
      ...
      if (detected){
        ...
        // show the corresponging neopixel and light up corresponding color, play a success tone
        CircuitPlayground.clearPixels();
        CircuitPlayground.playTone(800, 1000);
        light_LED(gesture, color_array[gesture]);
        
      } else {
        // Throws an error (lights up a red LED and play a fail tone)
        CircuitPlayground.clearPixels();
